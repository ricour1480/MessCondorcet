       ...
       if($('#quantiteUser').val() > 0 && $('#prod').val() !== "" && $('#idcate').val() !=="" ){
            ...
            .done(function(msg){
               if(Array.isArray(msg)){
                ...
                console.log(Panier);
                cpt++;
                panier+="<li id='"+id_produit+"'><img src='./admin/images/delete.jpg'  class='img-fluid dustbin'   id='supprime"+id_produit+"' width= 25px alt='dustbin'/><span class='nomproduit"+id_produit+"'>"+nom_produit+" </span>"+
                        "<span class='prixproduit"+id_produit+"'>Prix Total: "+prix_tot+" €</span>"+
                        "<span class='quantiteprod"+id_produit+"'> Qte : <input type='number' min='1' disabled value='"+quantite+"'  id='qteprod"+id_produit+"' /></li>";
